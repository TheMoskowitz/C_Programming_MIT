...
void inorder_traversal(p_tnode pnode, FILE * fp) {
	...
	for (n = 0; n < pnode->nkeys; n++) {
		...
		if (pnode->children[n]) inorder_traversal(pnode->children[n], fp);
		fprintf(fp, "%s\n", pnode->values[n]->name);
	}
	if (pnode->children[n] != NULL) {
...
int initialize_db (const char *filename) {
	...
	// Store all movies from database in B-Tree
	...
	rc = sqlite3_exec(db, sql, store_result, 0, &err_msg);
...
int locate_movie(const char *title) {
	...
	// Find a movie and print its record
...
void dump_sorted_list(const char *filename) {
	...
	if (ptreeroot == NULL) {
		fprintf(stderr, "Nothing to dump!\n");
	}
...
//     printf("Records stored in B-tree\n");
...
// 		fprintf(fp, "The movies in the database are:\n");

// 	    inorder_traversal(ptreeroot, fp);

// 	    fclose(fp);

// 	    printf("Information entered in %s\n", argv[2]);
...
// 	    printf("Enter a movie title for more information: ");
