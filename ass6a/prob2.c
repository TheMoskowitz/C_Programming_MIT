...
struct s_trie_node * new_node(void) {
	...
	memset(p_node->children, 0, sizeof(struct  s_trie_node*));
	return p_node;
...
void delete_node(struct s_trie_node * pnode) {
	/* TODO: delete node and all its children
	   ...
	   Hint: use recursion
	 */
	if (pnode->translation != NULL) free(pnode->translation);
	for (int i=0; i < UCHAR_MAX; ++i) {
		if (pnode->children[i] != 0) delete_node(pnode->children[i]);
	}
	free(pnode);
...
int add_word(const char * word, char * translation) {
	...
	for (int i=0; i < strlen(word); ++i) {
		if (ptr->children[word[i]] == 0) {
			ptr->children[word[i]] = new_node();
			new = 1;
		}
		ptr = ptr->children[word[i]];
	}

	if (ptr->translation == NULL) {
		ptr->translation = (char*)malloc(strlen(translation)+1);
		strcpy(ptr->translation, translation);
	} else {
		ptr->translation = realloc(ptr->translation, strlen(ptr->translation) + strlen(translation) + 2);
		ptr->translation = strcat(ptr->translation, ",");
		char *temp = (char*)malloc(strlen(translation)+1);
		strcpy(temp, translation);
		ptr->translation = strcat(ptr->translation, temp);
		free(temp);
	}
